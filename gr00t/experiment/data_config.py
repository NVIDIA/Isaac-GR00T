...
class UCRDataConfig(BaseDataConfig):
    video_keys = ["video.ego_view"]
    state_keys = [
        "state.waist_joint",
...
class UCRWBMDataConfig(BaseDataConfig):
    """
...
class UCRWBLMDataConfig(BaseDataConfig):
    """
    UCR data config with whole-body motion (WBM) actions.
    """
    ...
    def transform(self) -> ModalityTransform:
        ...
        return ComposedModalityTransform(transforms=transforms)
class UCRWBLMMobyDataConfig(BaseDataConfig):
    """
    UCR data config with whole-body motion (WBM) actions.
    """
