...
LEROBOT_RELATIVE_STATS_FILE_NAME = "relative_stats.json"  # Default filename


def get_relative_stats_filename(action_horizon: int) -> str:
    """Return the relative_stats filename corresponding to action_horizon.
    
    Prioritizes looking for relative_stats_{action_horizon}.json,
    falls back to relative_stats.json if it doesn't exist.
    """
...
class LeRobotEpisodeLoader:
    ...
    def __init__(
        ...
        # Extract action_horizon from modality_configs (used to load the correct relative_stats file)
        if "action" in modality_configs and modality_configs["action"].delta_indices:
            ...
            self.action_horizon = 16  # Default value
            print("*"*50)
    ...
    def _load_metadata(self) -> None:
        ...
        # Prioritize loading action_horizon-specific relative_stats file
        ...
        if relative_stats_path.exists():
            ...
            # Fall back to default relative_stats.json
