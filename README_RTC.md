...
```
7dfea4c - added: RTC in N1.6
a62040f - added standalone inference script  
7107b24 - added RTC in server-client mode
f9b0ae8 - add disable clip mode
88abfcb - update predict chunk size from 16 to 32
9fc5c32 - Make all comments in English
...
```
...
If you want to implement RTC in your own project, complete the following steps:

### Model Layer
- [ ] Add RTC parameters (`inference_rtc_overlap_steps`, `inference_rtc_frozen_steps`, `rtc_ramp_rate`)
- [ ] Modify action generation function to accept `action_input`
- [ ] Implement RTC initialization logic (override initial noise with previous action for the first overlap steps)
- [ ] Implement `vel_strength` control matrix (frozen + ramp + fully updated)
- [ ] Modify Euler integration update formula

### Policy Layer
- [ ] Modify observation processing logic to support `action` field
- [ ] Ensure action is correctly passed to the model layer

### Inference Code
- [ ] Implement action padding and overlap passing logic
- [ ] Correctly calculate `next_state_index`
- [ ] Consider whether to disable `clip_outliers`
...
| File | Description |
|------|-------------|
| `gr00t/model/gr00t_n1d6/gr00t_n1d6.py` | RTC core implementation |
| `gr00t/policy/gr00t_policy.py` | Policy layer action support |
| `gr00t/eval/run_gr00t_server.py` | Server configuration |
| `scripts/standalone_inference_RTC.py` | Standalone inference example |
| `scripts/test_piper_client_RTC.py` | Client-Server mode example |
