...
echo "Building $MODE model"

if [ $MODE == "full" ]; then
    # FULL MODEL
    echo "------------Building FULL MODEL--------------------"
    trtexec --fp16 --verbose  --separateProfileRun --noDataTransfers --onnx=gr00t_onnx/full_model/full_model.onnx --saveEngine=gr00t_engine/full_model.engine --minShapes=pixel_values:1x3x224x224,input_ids:1x${MIN_LEN},attention_mask:1x${MIN_LEN},state:1x1x64,embodiment_id:1,init_actions:1x16x32 --optShapes=pixel_values:1x3x224x224,input_ids:1x${OPT_LEN},attention_mask:1x${OPT_LEN},state:1x1x64,embodiment_id:1,init_actions:1x16x32 --maxShapes=pixel_values:8x3x224x224,input_ids:8x${MAX_LEN},attention_mask:8x${MAX_LEN},state:8x1x64,embodiment_id:8,init_actions:8x16x32 > gr00t_engine/full_model.log 2>&1
else 
